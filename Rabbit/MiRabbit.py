...
MY_IMAGE = Image.open('discord.png')
MESSAGE = MY_IMAGE.tobytes()
...
MESSAGE_2 = Image.open('result.png').tobytes().hex()
...
if(OPTION == "E"):
	...
	print(result)
	imageResult = Image.frombytes(MY_IMAGE.mode, MY_IMAGE.size, bytes(result.encode('utf-8')))
	imageResult.save('result.png')

else:
	result = decryptMessage(MESSAGE_2)
	print(result)
	base64_bytes = base64.b64encode(result)
	imageResult = Image.frombytes(MY_IMAGE.mode, MY_IMAGE.size, base64_bytes)
	imageResult.save('result-2.png')