...
if __name__ == "__main__":
    ...
    r = Rabbit(123456789123456, 0xA6EB561AD2F41727)
    key = r.next().derive()
    key_bytes = key.to_bytes(16, 'big')
    key_array = bytearray(key.to_bytes(16, 'big'))
    
    original_image = Image.open("discord.png")
    original_image_array = bytearray(original_image.tobytes())

    result_bytes = [None]*len(original_image_array)

    for i in range(len(original_image_array)):
        result_bytes[i] = xor(original_image_array[i], key_bytes)
    
    encript_bytes = b''.join(result_bytes)

    result = Image.frombytes(original_image.mode, original_image.size, encript_bytes)
    ...
    result_bytes = [None]*len(encripted_image_array)

    for i in range(len(encripted_image_array)):
        result_bytes[i] = xor(encripted_image_array[i], key_bytes)

    decripted_bytes = b''.join(result_bytes)
